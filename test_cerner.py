"""
Test Script for Cerner FHIR Lab Report Service (Bulk Data Version)

This script tests the Bulk Data Access components:
1. Configuration & Auth (Cerner-specific JWT claims)
...
from cerner_fhir_client import CernerFHIRClient, EmailSender, LabReportService # ASSUMES CernerFHIRClient is in cerner_fhir_client.py
...
def test_config():
    ...
    try:
        ...
        # Check required fields for Cerner
        required_cerner = ["client_id", "private_key_path", "token_url", "fhir_base_url", "key_id"]
        required_email = ["smtp_host", "smtp_port", "smtp_user", "smtp_password", "from_email", "to_email"]
        
        for field in required_cerner:
            if field not in config.get("cerner", {}):
                print(f"❌ Missing cerner.{field} in config. NOTE: Cerner requires 'key_id' (kid)!")
                return None
        print("✓ Cerner configuration fields present")
        
        # Check key_id (CRITICAL for Cerner JWT header)
        if "key_id" not in config["cerner"] or not config["cerner"]["key_id"]:
            print("❌ CRITICAL: cerner.key_id is missing or empty!")
            print("   Cerner REQUIRES the 'kid' (key ID) in the JWT header to match your JWKS configuration.")
            return None
        print(f"✓ Key ID (kid) set to: {config['cerner']['key_id']}")
        ...
        if "group_id" not in config or not config["group_id"]:
            ...
            print("   The Bulk Data API (Group Export) requires a specific FHIR Group ID.")
            return None
...
def test_private_key(config):
    ...
    key_path = Path(config["cerner"]["private_key_path"])
...
def test_jwt_creation(config):
    """Test JWT creation with Cerner-specific claims."""
    print("\n" + "="*50)
    print("TESTING JWT CREATION (Cerner Claims)")
    print("="*50)
    ...
    try:
        ...
        with open(config["cerner"]["private_key_path"], 'r') as f:
            private_key = f.read()
        
        now = int(time.time())
        # Cerner Specific: 'aud' must match token_url exactly
        token_url = config["cerner"]["token_url"]
        client_id = config["cerner"]["client_id"]
        key_id = config["cerner"]["key_id"]
        ...
        claims = {
            ...
            "aud": token_url,
            "jti": str(uuid.uuid4()),
            "exp": now + 280  # Cerner recommends <300 seconds
        }
        
        # Cerner Specific: 'kid' in header is MANDATORY
        headers = {
            "alg": "RS384",
            "typ": "JWT",
            "kid": key_id
        ...
        if header.get('kid') == key_id:
            print(f"  ✓ kid is present in JWT header (kid={key_id})")
        else:
            print("  ❌ kid is MISSING or MISMATCHED in JWT header!")
...
def test_authentication(config):
    ...
    print("\n" + "="*50)
    print("TESTING CERNER AUTHENTICATION")
    print("="*50)
    
    try:
        # CernerFHIRClient is imported from cerner_fhir_client.py
        client = CernerFHIRClient(config["cerner"])
        ...
        if client.authenticate():
            ...
            print("❌ Authentication failed. Check logs for Cerner response.")
            return None
...
def test_bulk_export_capabilities(client, config):
    ...
    Does NOT wait for the full download, but verifies we can Start and Monitor the job.
    """
    print("\n" + "="*50)
    print("TESTING CERNER BULK DATA EXPORT HANDSHAKE")
    print("="*50)
...
def test_email(config):
    ...
    try:
        # EmailSender is imported from cerner_fhir_client.py
        sender = EmailSender(config["email"])
        
        test_subject = "Cerner FHIR Test"
        test_text = "Test email from setup script."
        test_html = "<html><body><h1>Cerner Test Email</h1><p>Test email from setup script.</p></body></html>"
...
def run_full_test(config):
    ...
    print("\n" + "="*50)
    print("RUNNING FULL INTEGRATION TEST (Cerner)")
    print("WARNING: This may take several minutes to complete the Bulk Export.")
    print("="*50)
    
    try:
        # All required classes are imported at the top
        
        fhir_client = CernerFHIRClient(config["cerner"])
...
def main():
    """Main test runner."""
    print("\n" + "#"*60)
    print("#   CERNER FHIR BULK DATA SERVICE - TEST SUITE")
    print("#"*60)
